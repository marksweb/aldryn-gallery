    ...
    def render(self, context, instance, placeholder):
        ...
        if instance.child_plugin_instances:
            number_of_slides = sum(
                [plugin.folder.file_count if
                    isinstance(plugin, SlideFolderPlugin) else 1
...
class SlideFolderCMSPlugin(GalleryChildBase):
    ...
    def render(self, context, instance, placeholder):
        ...
        if settings.ALDRYN_BOILERPLATE_NAME == 'legacy':
            context['slide_template'] = self.get_slide_template(
                instance=instance, name='slide'
            )
        else:
            # for 'bootstrap3' boilerplate and the recommended
            # structure for other boilerplates
            context['slide_template'] = self.get_slide_template(
                instance=instance, name='image_slide'
            )
        return context
    ...
    def get_render_template(self, context, instance, placeholder):
        if settings.ALDRYN_BOILERPLATE_NAME == 'legacy':
            ...
            # for 'bootstrap3' boilerplate and the recommended
            # structure for other boilerplates
            return self.get_slide_template(
                instance=instance, name='slide_folder'
            )
